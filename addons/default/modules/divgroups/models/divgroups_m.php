...
class Divgroups_m extends MY_Model
{
	protected $_table = 'default_division_groups';
	...
	public function insert($input = array())
	{
		parent::insert(array(
			...
			'division_group_name' 			=> $input['division_group_name'],
			'approver' 						=> $input['approver'],
			'approver_proxy' 				=> $input['approver_proxy'],
			'home_warehouse' 				=> $input['home_warehouse']
		));
	...
	public function get_all()
	{
		...
		$this->db
			->select('division_groups.*,profiles.first_name AS approver_fname,profiles.last_name AS approver_lname')
			->join('profiles', 'division_groups.approver = profiles.user_id', 'left');
		return $this->db->get('division_groups')->result();
	...
	public function check_if_has_division($id = 0)
	{
		$this->db
			->select('divisions.*')
	...
	public function get_division_group_approvers()
	{
		$this->db
