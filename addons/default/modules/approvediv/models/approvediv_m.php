...
class Approvediv_m extends MY_Model
{
	...
	public function get_approver_division($id)
	{
		return $this->db
			->select('divisions.*')
			->where('approver',$id)
	...
	public function is_division_approver($id)
	{
		$this->db->where(array('id' => $id));
		
		$user_info = $this->db->get('users')->row();
		
		$is_approver = ($user_info->group_id == '8') ? 1 : 0;
	...
	public function count_by_division($id)
	{
		return $this->db
			->select('material_requests.*')
			->where(array('division' => $id))
			->count_all_results('material_requests');
	}
