...
class Admin extends Admin_Controller
{
	...
	protected $validation_rules = array(
        ...
        array(
			'field' => 'description',
			'label' => 'lang:blog:description_label',
			'rules' => 'trim|required|max_length[50]'
        ),
        array(
			'field' => 'long_description',
			'label' => 'lang:blog:long_description_label',
        ...
        array(
			'field' => 'unit',
			'label' => 'lang:blog:unit_label',
			'rules' => 'trim|required'
        ),
        array(
			'field' => 'volume',
			'label' => 'lang:blog:volume_label',
			'rules' => 'trim|numeric'
        ),
        array(
			'field' => 'weight',
			'label' => 'lang:blog:weight_label',
			'rules' => 'trim|numeric'
        ),
        array(
			'field' => 'is_serialized',
			'label' => 'lang:blog:serialized_label',
			'rules' => 'trim|required'
        )
	);
	...
	public function __construct()
	{
		...
		$this->load->model(array('item_m','categories/category_m'));
		$this->lang->load(array('item', 'categories/category','blog'));
		...
		$_categories = array();
		if ($categories = $this->category_m->order_by('id')->get_all())
		{
			foreach ($categories as $category)
			{
				$_categories[$category->id] = $category->id;
			}
		}
...
//~ 
		...
		$pagination = create_pagination('admin/items/index', $total_rows);
		//~ print_r($pagination);
		//~ die();
//~ 
...
//~ 
		...
		if ($this->form_validation->run())
		{
			...
			//Generate Item Barcode. This should only be done once and is unchangable in the future. 
			$item_barcode = $this->generate_barcode($this->input->post('category'));
			
			if ($id = $this->item_m->insert(array(
				'description'		=> $this->input->post('description'),
				'long_description'	=> $this->input->post('long_description'),
				'category_id'		=> $this->input->post('category'),
				'unit'				=> $this->input->post('unit'),
				'volume'			=> $this->input->post('volume') == '' ? '0.0000' : $this->input->post('volume'),
				'weight'			=> $this->input->post('weight') == '' ? '0.0000' : $this->input->post('weight'),
				'is_serialized'		=> $this->input->post('is_serialized'),
				'barcode'			=> $item_barcode['whole'],
				'barcode_alpha'		=> $item_barcode['alpha'],
				'barcode_num'		=> $item_barcode['num'],
			...
			{
				$this->pyrocache->delete_all('item_m');
				$this->session->set_flashdata('success', sprintf($this->lang->line('blog:post_add_success'), $this->input->post('description')));
			...
			$this->input->post('btnAction') == 'save' ? redirect('admin/items') : redirect('admin/items/edit/' . $id);
		}
		else
		{
    ...
    public function generate_barcode($category_id)
    {
		
		$category_code = $this->category_m->get($category_id)->category_code;
		$barcode['alpha'] = $category_code;
		
		if($last_code_in_category = $this->item_m->get_last_category_no($category_code))
			{	
				$last_code = $last_code_in_category->barcode_num;
				$barcode['num'] = $last_code + 1;
			}
		else 
			{	
				$barcode['num'] = 1;  
			...
			$barcode['whole'] = $barcode['alpha'].$barcode['num'];
	...
	{
		$id OR redirect('admin/items');
		
		$item_details = $this->item_m->get($id);
		
		$this->template
			->title($this->module_details['name'], lang('item:select_title'))
			->append_metadata($this->load->view('fragments/wysiwyg', $this->data, TRUE))
			->append_js('jquery/jquery.tagsinput.js')
			->append_js('module::blog_form.js')
			->append_css('jquery/jquery.tagsinput.css')
			->set('item', $item_details)
			->build('admin/select');
