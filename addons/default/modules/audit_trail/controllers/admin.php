...
class Admin extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array('audit_trail_m','special_approver/special_approvegroup_m','divgroups/divgroups_m','approvedivgroup/approvegroup_m','materialrequest/matreq_m','category_m', 'blog_categories_m','approvediv/approvediv_m','users/user_m','items/item_m'));
		$this->lang->load('audit_trail');
	...
	public function index()
	{
		...
		$pagination = create_pagination('admin/audit_trail/index', $total_rows, NULL, 4);
		...
		$material_requests = $this->matreq_m->get_all();
		...
		$this->input->is_ajax_request()
			? $this->template->build('admin/audit_trail/index')
	...
	{	// is ID specified
		$id OR redirect('admin/audit_trail');
		
		//get mr
		$mr = $this->matreq_m->get($id);
		
		// mr exist?
		$mr OR redirect('admin/audit_trail');
		...
		$this->input->is_ajax_request()
			...
			: $this->template->build('admin/audit_trail/view_mr');

	}
	function require_changes($mr_id)
	{			
		$mr_id OR redirect('admin/audit_trail');
		// Get the unit			
	...
	{			
		$stat OR redirect('admin/audit_trail');
		...
		if($mr->status == 3)
		{echo "<script>alert('mrstat')</script>";
			if($stat == 'hold')
			{
				$INPUT = array(
				'status' 			=> 7,
