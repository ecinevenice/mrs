...
class audit_trail_m extends MY_Model
{
	protected $_table = 'default_notifications';

	
	
	
	public function get_mr_history($id)
	{
		return $this->db
			->select('default_notifications.*')
			->where('relative_id',$id)
			->get('default_notifications')
			->result();
	}	
	...
	public function get_mr_history_actions($action_id)
	{
		return $this->db
			->select('default_notification_types.*')
			->where('id',$action_id)
			->where('relative_type','MR')
			->get('default_notification_types')
			->row();
	}
	
	
	public function insert_to_history($data)
	{
		$this->db->insert('default_notifications', $data);
	}
	/**
