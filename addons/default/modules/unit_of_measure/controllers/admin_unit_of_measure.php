 ...
 */
class Admin_unit_of_measure extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('blog_unit_of_measure_m');
	...
	public function index()
	{
		...
		$total_rows = $this->blog_unit_of_measure_m->count_all();
		$pagination = create_pagination('admin/blog/unit_of_measure/index', $total_rows, NULL, 5);
			
		// Using this data, get the relevant results
		$unit_of_measure = $this->blog_unit_of_measure_m->order_by('title')->limit($pagination['limit'])->get_all();
	...
	public function create()
	{
		...
		{
			if ($id = $this->blog_unit_of_measure_m->insert($_POST))
			{
			...
			redirect('admin/blog/unit_of_measure');
		}
	...
	{	
		// Get the category
		$category = $this->blog_unit_of_measure_m->get($id);
		...
		if ($this->form_validation->run())
		{		
			$this->blog_unit_of_measure_m->update($id, $_POST)
			...
			redirect('admin/blog/unit_of_measure/index');
		}
	...
	{	
		...
		if (!empty($id_array))
		{
			...
			{
				if ($this->blog_unit_of_measure_m->delete($id))
				{
					$deleted++;
					$deleted_ids[] = $id;
				}
			...
			// Fire an event. One or more unit_of_measure have been deleted.
	...
	public function _check_title($title = '')
	{
		...
		if ($this->blog_unit_of_measure_m->check_title($title,$id))
		{
	...
	public function create_ajax()
	{
		...
		if ($this->form_validation->run())
		{
			$id = $this->blog_unit_of_measure_m->insert_ajax($this->input->post());
		...
		{
			// Render the view
			$form = $this->load->view('admin/unit_of_measure/form', $data, TRUE);
