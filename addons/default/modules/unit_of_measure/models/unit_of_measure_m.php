...
class Unit_of_measure_m extends MY_Model
{
	protected $_table = 'default_unit_of_measure';
	...
	public function get_all()
	{
		...
		$this->db
			...
			->order_by('id','ASC');
		return $this->db->get('default_unit_of_measure')->result();
	...
	public function count_category_items($id)
	{
		return $this->db
			->select('unit_of_measure.*')
			->where(array('id' => $id))
			->count_all_results('unit_of_measure');
	}
	...
	public function check_exists_other($field, $value)
	{
		if (is_array($field))
		{
			$params = $field;
