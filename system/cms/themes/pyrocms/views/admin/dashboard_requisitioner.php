		...
		<section class="item">
			<ul >
				<?php if ($material_request): ?>
						<?php foreach ($material_request AS $mr) : ?>
							<li>
								<p><?php 
								if($mr->status == 1) 
									echo 'Saved <a href="admin/materialrequest/add_items/'.$mr->id.'">'.$mr->title.'</a> to draft.';								
								if($mr->status == 2) 
									echo 'Submitted <a href="admin/materialrequest/view_mr/'.$mr->id.'">'.$mr->title.'</a> for approval';
								?></p>
							</li>
						<?php endforeach; ?>
				<?php else: ?>
